...
imageFolder2=[dataFolder filesep 'CLstraight'];
mkdir(imageFolder);
mkdir(imageFolder2);
counter=1;
show=0;
...
stackRange=435:445;% startStack:endStack;
...
[~,ia,ib]=unique(fluorIdx);
       ...
       fluorFrame3=imwarp(fluorFrame,(lowResFluor2BF.t_concord),...
                'OutputView',lowResFluor2BF.Rsegment);
...
for i=1:zSize
    midZplotVal=max(pdist(find(bwmorph(segmentChannel4(:,:,i),'remove'))));
...
[CL2(:,2,:),CL2(:,1,:)]=transformPointsInverse(hiResFix.t_concord,CL2(:,2,:),CL2(:,1,:));
...
%%
midImS=fluorFrame2(:,:,midZ)-worm2(:,:,midZ);
minSearch=[];
for i=1:length(CL3)
...
zslice=permute(zslice,[3,2,1]);
 ...
 %       if 1%~cline_para.gradflag
            ...
            lowResV=interp3(fluorFrame2,xslice,yslice,zslice,'*linear',0);
