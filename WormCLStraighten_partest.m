function [V,pointStats,Vproj,vRegion]=WormCLStraighten_partest(dataFolder,destination,vidInfo,alignments,ctrlPoints,Vtemplate,vRegion,zOffset,iStack,show)
...
display([ 'Initialization done in ' num2str(round(toc))]);
...
display([ 'Straightening done in ' num2str(round(toc))]);

   %% stack stabilization
   
   [V, tformAll]=stackStabilization(V,30,show);
...
display([ 'Stabilizing done in ' num2str(round(toc))]);
...
display([ 'Shift done in ' num2str(round(toc))]);
...
boundary2=[aOut2(1)*(xVec2-outputRadius-1).^2+aOut2(2)*(xVec2-outputRadius-1)+nring2 ...
 ...
 display([ 'Regions done in ' num2str(round(toc))]);
...
%     fiducialsAll(iStack).model=model;
 ...
 %   pointStats(counter).Gintensities=intensities;
  ...
  display([ 'Segmentation done in ' num2str(round(toc))]);
