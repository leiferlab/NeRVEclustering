function  imFlash=findDatFlash(datFile,rows)
% function takes a .dat file for worm data and searches each of the images
% for a flash. The images must be 1024x1024 matrix of uint16, works super
...
stackSize=ftell(Fid)/(2*1024^2)-1;
status=fseek(Fid,0,-1);
else
...
for iFrame=1:stackSize
    ...
    pixelValues=fread(Fid,1024*rows,'uint16',0,'l');
    status=fseek(Fid,2*1024*(1024-rows),0);
