...
TrackMatrixTemp=TrackMatrixTemp(:,selectionRange);
          validPoints=TrackMatrixTemp(:)>0;
       % [outRangeoverlap,ia, ib]=intersect(outRange2{j},outRange2{i});
      startIdx=indexAdd(outRange(i));
        TrackMatrixTemp=bsxfun(@plus,TrackMatrixTemp , indexAdd2(1:end-1));
      startIdx=indexAdd(outRange(i));
        startPos=startIdx+(1:size(TrackMatrixTemp,1));
        startPos=repmat(startPos,length(selectionRange),1)';
...
%     transitionMatrixi=double(transitionMatrixi);
%     
subTranstionMatrix=transitionMatrixi(1:indexAdd(175),:);
