...
hasPoints=find(hasPoints>max(hasPoints)*.8);
...
if any(eigIdx)
    temp=centerlineFile.(fieldNames{eigIdx});
else
    temp=load([dataFolder filesep 'BehaviorAnalysis' filesep 'eigenProj']);
    fieldNames=fieldnames(temp);
temp=temp.(fieldNames{1});
...
hiResCLV=interp1(bfAll.frameTime,CLV,hiResData.frameTime);
...
for i=1:size(RvalsAll,1)
    ...
    present=(~isnan(RvalsAll(i,:)+GvalsAll(i,:))' & xVals>200) ;
