...
# Number of Frames
master.L6 = Label(master, text="groups per frame")
master.L6.grid(row=5, column=0, sticky=W+E)

master.e6 = Entry(master)
master.e6.insert(0,'1')
master.e6.grid(row=5, column=1, sticky=W+E)

#Slurm or SGE?
master.L7 = Label(master, text="Use Slurm")
master.L7.grid(row=6, column=0, sticky=W+E)

var3 = IntVar();
master.CB7 = Checkbutton(master, text=None, variable=var3);
...
def submitScript(master=None):
    ...
    print("Number of Files is" + groupSize);
    nGroups=master.e6.get();
    print("Number of Groups is " + nGroups)
    totalRuns=str(int(groupSize)*int(nGroups))
    ...
    if master.CB7.var3.get()==1:
       slurmFlag = 1;
       server='gen-comp1.princeton.edu'
       print("Request: Use Slurm")
    else:
       slurmFlag = 0;
       server='cetus.princeton.edu'
       print("Request: Use SGE")
...
#"20150616/BrainScanner20150616_100353-worm1"
    	qsubCommand = ("sbatch --mem=204800 --qos=" + queueUsed + " -D " + folderName
    		+ " --output =\"" + "slurmJob-%J.out" + "\" --error=\"" 
    		+ "slurmJob-%J.err" + "\" --array=1-" + totalRuns + ":" + str(1) 
    		+ " ../scripts/shae-pythonSubmissionScripts/runWormCellTracking.sh '" 
    		+ matlabDirName +"' " + nGroups + " 1");
    else:
...
master.b.grid(row=7,columnspan=2, sticky=W+E)
