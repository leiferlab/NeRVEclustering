...
se=strel('disk',50);
imageFolder2=[dataFolder filesep 'CLstraight_20150521'];
mkdir(imageFolder2);
counter=1;
show=0;
 ...
 tic
   ...
   segmentChannel2=[]; segmentChannel3=[]; segmentChannel4=[]; segmentChannel5=[];
    ...
    zRange=hiResData.Z(hiResIdx-zOffset);
          ...
          bfIdx=round(bfIdxLookup(hiResIdx-zOffset));
            fluorIdx=round(fluorIdxLookup(hiResIdx-zOffset));
...
fluorProj=normalizeRange(sum(fluorFrame2,3));
hiResProj=normalizeRange(sum(segmentChannel,3));

    corrIm=xcorr2(fluorProj,hiResProj);
    [CLoffsetX,CLoffsetY]=find(corrIm==max(corrIm(:)));
...
[~,midZ]=max(smooth(midZplot3+midZplot,3));
%%
...
%%
hiResIdxStretch=min(hiResIdx)-50:max(hiResIdx)+50;
...
[CL2(:,1,:),CL2(:,2,:)]=transformPointsForward(Hi2LowResF.t_concord,CL(:,2,:),CL(:,1,:));

CL2(:,2,:)=CL2(:,2,:)-600;

%[CL2(:,1,:),CL2(:,2,:)]=transformPointsInverse(hiResFix.t_concord,CL2(:,2,:),CL2(:,1,:));
...
CL3temp=interp1(CL3temp,shiftVec(ic(iCL))+(-500:2500),'linear','extrap');
    CL3all2(:,:,iCL)=CL3temp;
        ...
        plot(CL3temp(:,1),CL3temp(:,2))
      % scatter(CL3temp(:,1),CL3temp(:,2),[],(1:length(CL3temp)))
    drawnow
    end
...
inImageRange=round(mean(find(inImage)))+(-outputLength:outputLength);
%%
...
minZ=find(minSearch2==min(minSearch2(1:size(CL3all,3)/2)),1,'first');
...
minSearch2=smooth(minSearch2,4);
maxZ=find(minSearch2==min(minSearch2(round(size(CL3all,3)/2):end)),1,'first');
...
wormProj=smooth2a(sum(V(:,:,round(outputRadius+(-20:20))),3),1,1);
...
W=max(0,(maxY-outputRadius).*wormProjMaxI);
maxX(W==0)=[];
maxY(W==0)=[];
W(W==0)=[];
if ~isempty(maxX)
...
%yshift(yshift>(outputRadius*1.5))=(outputRadius*1.5);
