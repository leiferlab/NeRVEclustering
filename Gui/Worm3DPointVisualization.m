function varargout = Worm3DPointVisualization(varargin)
% WORM3DPOINTVISUALIZATION MATLAB code for Worm3DPointVisualization.fig
%      WORM3DPOINTVISUALIZATION, by itself, creates a new WORM3DPOINTVISUALIZATION or raises the existing
%      singleton*.
%
%      H = WORM3DPOINTVISUALIZATION returns the handle to a new WORM3DPOINTVISUALIZATION or the handle to
%      the existing singleton*.
%
%      WORM3DPOINTVISUALIZATION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WORM3DPOINTVISUALIZATION.neuron6 with the given input arguments.
%
%      WORM3DPOINTVISUALIZATION('Property','Value',...) creates a new WORM3DPOINTVISUALIZATION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Worm3DPointVisualization_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Worm3DPointVisualization_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Worm3DPointVisualization

% Last Modified by GUIDE v2.5 05-Nov-2015 14:27:29

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @Worm3DPointVisualization_OpeningFcn, ...
    'gui_OutputFcn',  @Worm3DPointVisualization_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before Worm3DPointVisualization is made visible.
function Worm3DPointVisualization_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Worm3DPointVisualization (see VARARGIN)

% Choose default command line output for Worm3DPointVisualization
...
% UIWAIT makes Worm3DPointVisualization wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Worm3DPointVisualization_OutputFcn(hObject, eventdata, handles)
...
set(handles.timeOffset,'String',num2str(zOffset));
...
%load centerlines and alignemnts!
if isdir(dataFolder)
...
bfIdxLookup(bfIdxLookup>length(centerline)|isnan(bfIdxLookup))=length(centerline);
[CL2(:,2,:),CL2(:,1,:)]=transformPointsInverse(lowResFluor2BF.t_concord,centerline(:,2,:),centerline(:,1,:));

%[CL2(:,1,:),CL2(:,2,:)]=transformPointsForward(Hi2LowResF.t_concord,CL2(:,2,:),CL2(:,1,:));
[CL2(:,1,:),CL2(:,2,:)]=transformPointsForward(Hi2LowResF.t_concord,CL2(:,2,:),CL2(:,1,:));
CL2(:,2,:)=CL2(:,2,:)-(rect1(2)-1);
CL3=CL2(:,:,round(bfIdxLookup));

setappdata(handles.figure1,'centerlines',CL3);
end
...
centerlines=getappdata(handles.figure1,'centerlines');
hiResData=getappdata(handles.figure1,'hiResData');
...
plot(handles.axes1,centerlines(:,1,hiResIdx),centerlines(:,2,hiResIdx),'r',...
    'LineWidth',3)
axis(handles.axes1,'equal');
xlim(handles.axes1,[1 size(baseImg,1)]);
ylim(handles.axes1,[1 size(baseImg,2)]);
