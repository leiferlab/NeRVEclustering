function varargout = VisualizeWormDataSplit3D3(varargin)
% VISUALIZEWORMDATASPLIT3D3 MATLAB code for VisualizeWormDataSplit3D3.fig
%      VISUALIZEWORMDATASPLIT3D3, by itself, creates a new VISUALIZEWORMDATASPLIT3D3 or raises the existing
%      singleton*.
%
%      H = VISUALIZEWORMDATASPLIT3D3 returns the handle to a new VISUALIZEWORMDATASPLIT3D3 or the handle to
%      the existing singleton*.
%
%      VISUALIZEWORMDATASPLIT3D3('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VISUALIZEWORMDATASPLIT3D3.M with the given input arguments.
%
%      VISUALIZEWORMDATASPLIT3D3('Property','Value',...) creates a new VISUALIZEWORMDATASPLIT3D3 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before VisualizeWormDataSplit3D3_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to VisualizeWormDataSplit3D3_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help VisualizeWormDataSplit3D3

% Last Modified by GUIDE v2.5 29-Oct-2014 14:55:36

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @VisualizeWormDataSplit3D3_OpeningFcn, ...
    'gui_OutputFcn',  @VisualizeWormDataSplit3D3_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before VisualizeWormDataSplit3D3 is made visible.
function VisualizeWormDataSplit3D3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to VisualizeWormDataSplit3D3 (see VARARGIN)

% Choose default command line output for VisualizeWormDataSplit3D3
...
% UIWAIT makes VisualizeWormDataSplit3D3 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = VisualizeWormDataSplit3D3_OutputFcn(hObject, eventdata, handles)
...
setappdata(0,'registration',registration);
    imFiles=rawImFolder;
    dataFolder=fileparts(imFiles{1});
...
maxFrame=max(trackOutput(:,end-1));
minFrame=1;
...
if length(imFiles)==1
    ...
    imFiles=imFiles{1};
    metaData=wormData.metaData;
    zPos=metaData.zVoltage;
            zPosV=(get(handles.zSlider,'Value'));
           set(handles.zSlider,'Max',max(zPos))
        set(handles.zSlider,'Min',min(zPos))
   %     set(handles.zSlider,'Value',max(zPosV,
   %deal with ocasional hiccups with zvoltage non monotonic
   if zPos(1)>zPos(end);
       zPos=sort(zPos,'ascend')+(1:length(zPos))'*.00001;
   else zPos=sort(zPos,'descend')-(1:length(zPos))'*.00001;
   end
   
        zSlice=interp1(zPos,1:length(zPos),zPosV,'nearest','extrap');
        zSlice=round(zSlice);
    %load image and correct pixels
    [zPosV,zSlice]
%    hiResIdx=metaData.iFrame(zSlice);
    hiResIdx=find(hiResData.stackIdx==iImage,zSlice,'first');
    hiResIdx=hiResIdx(end)-11;
    Fid=fopen(imFiles);
    ...
    labelMask=wormData.wormMask(:,:,zSlice);
    ...
    hold(handles.axes1,'off')
        ...
        otherwise
                baseImg=[];
                centroids(:,3)=centroids(:,3)-median(centroids(:,3))+50;
    ...
    if ismatrix(wormMask)
        ...
        zOff=interp1(zPos,midPlane);
        zOff=median(wormData.zPlaneIdx(wormData.centroids(:,2)>400));
        zOff=metaData.zVoltage(round(zOff));
...
%         baseImg = (baseImg-newContrast(1))./diff(newContrast);
    ...
    if flag3d
        scat=scatter(handles.axes1,centroids(tracks(:,end-2),1),...
            centroids(tracks(:,end-2),2),'rx');
    else
        scat=scatter(handles.axes1,centroids(tracks(:,end-2),1),...
            centroids(tracks(:,end-2),2),'rx');
    end
    ...
    text( centroids(tracks(:,end-2),1), centroids(tracks(:,end-2),2),cellstr(num2str(tracks(:,end))),'VerticalAlignment'...
...
%            a2=normalizeRange(smooth(a2,smoothWindow))+i-1;
            a=(smooth(a/nanmedian(a),smoothWindow))/5+i-1;
    ...
    subIdxc=tracks(subIdx,end-2);
    text(centroids(subIdxc,1),centroids(subIdxc,2),cellstr(num2str(tracks(subIdx,end))),'VerticalAlignment'...
