function res = bpass3_z(image_array,lnoise,lnoisez,lobject,threshold, isForcedDouble)
%%% EDITED BY SAGAR SETRU, 16 NOV 2014
%%% Code was edited to have a separate distinct noise scale in z
...
%               Image Processing
% CALLING SEQUENCE:
%               res = bpass( image_array, lnoise, lnoisez, lobject )
% INPUTS:
...
if lnoisez == 0
  ...
  gaussian_kernelz = normalize(...
    exp(-((-ceil(5*lnoisez):ceil(5*lnoisez))/(2*lnoisez)).^2));
end
...
gconv = convnfft(image_array,gaussian_kernel,'same');

gaussian_kernel2 = repmat(gaussian_kernel,[length(gaussian_kernel),1,length(gaussian_kernel)]);
gaussian_kernel3 = gaussian_kernel2.*permute(gaussian_kernel2,[3,1,2]).*permute(gaussian_kernel2,[2,3,1]);

gaussian_kernel2t = repmat(gaussian_kernel,[length(gaussian_kernel),1,length(gaussian_kernelz)]);
if length(gaussian_kernel) > length(gaussian_kernelz),
    padDiff = length(gaussian_kernel) - length(gaussian_kernelz);
    padAdd = zeros(length(gaussian_kernel),length(gaussian_kernel),padDiff/2);
    gaussian_kernel2t = cat(3,padAdd,gaussian_kernel2t,padAdd);
else
    padDiff = length(gaussian_kernelz) - length(gaussian_kernel);
    padAdd1 = zeros(length(gaussian_kernel),padDiff/2,length(gaussian_kernelz));
    padAdd2 = zeros(padDiff/2,length(gaussian_kernelz),length(gaussian_kernelz));
    gaussian_kernel2t = cat(2,padAdd1,gaussian_kernel2t,padAdd1);
    gaussian_kernel2t = cat(1,padAdd2,gaussian_kernel2t,padAdd2);
end
gaussian_kernel3t = gaussian_kernel2t.*permute(gaussian_kernel2t,[3,1,2]).*permute(gaussian_kernel2t,[2,3,1]);
...
if lobject
    ...
    boxcar_kernel = repmat(boxcar_kernel,[length(boxcar_kernel),1,length(boxcar_kernel)]);
    boxcar_kernel = boxcar_kernel.*permute(boxcar_kernel,[3,1,2]).*permute(boxcar_kernel,[2,3,1]);

    bconv = convnfft(image_array, boxcar_kernel,'same');
    filtered = gconv - bconv;
else
    filtered = gconv;
end
