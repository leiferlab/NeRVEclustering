...
xlswrite([dataFolder filesep 'excelData' filesep 'activity.xlsx'],Ratio2','Activity')
...
for i=1:size(G2,1);
 ...
 %   gTau(i)=rTau(i);
    ...
    deltabootGP(i,iTrials)=nanmean(gperm(behaviorP))-nanmean(gperm(~behaviorP));
      ...
      if gflag
    rperm=rtemp(randpermchunks(length(rtemp),2*gTau(i)));
...
possibleB=find(((GcorrB)>GbootCorrSTDB) & GcorrB>=corrThresh...
...
possibleF=find(GcorrF>corrThresh & ((GcorrF)>GbootCorrSTDF)...
...
possibleP=find(((GcorrP)>GbootCorrSTDP) & GcorrP>corrThresh  ...
...
possibleT=find(((GcorrT)>GbootCorrSTDT) & GcorrT>corrThresh ...
...
allPoints=find(nPoints==max(nPoints-1));
