...
%% using resulting V, do a rotation to try to straighten things out a bit
Vproj=sum(V,3);
theta=-50:1:50;
[~,peakPos]=max(sum(Vproj,2)); %find bright point in profile, go down 100
peakPos=min(peakPos+100,size(Vproj,1));

VR=radon(Vproj(1:peakPos,:),theta);
[~,stdPeak]=max(std(VR));
stdPeak=-theta(stdPeak);
% xcenter=sum(Vproj,1);
% xcenter=xcenter-min(xcenter);
% xcenter=mean(find(xcenter>(max(xcenter/2))));
% translation=-floor(outputRadius-xcenter);

if abs(stdPeak)>2

    V=imrotate(V,stdPeak,'crop');
    xslice=imrotate(xslice,stdPeak,'crop');
    yslice=imrotate(yslice,stdPeak,'crop');
end
