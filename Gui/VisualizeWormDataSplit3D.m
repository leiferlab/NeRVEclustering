function varargout = VisualizeWormDataSplit3D(varargin)
% VISUALIZEWORMDATASPLIT3D MATLAB code for VisualizeWormDataSplit3D.fig
%      VISUALIZEWORMDATASPLIT3D, by itself, creates a new VISUALIZEWORMDATASPLIT3D or raises the existing
%      singleton*.
%
%      H = VISUALIZEWORMDATASPLIT3D returns the handle to a new VISUALIZEWORMDATASPLIT3D or the handle to
%      the existing singleton*.
%
%      VISUALIZEWORMDATASPLIT3D('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VISUALIZEWORMDATASPLIT3D.M with the given input arguments.
%
%      VISUALIZEWORMDATASPLIT3D('Property','Value',...) creates a new VISUALIZEWORMDATASPLIT3D or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before VisualizeWormDataSplit3D_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to VisualizeWormDataSplit3D_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help VisualizeWormDataSplit3D

% Last Modified by GUIDE v2.5 30-Sep-2014 09:51:53

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @VisualizeWormDataSplit3D_OpeningFcn, ...
    'gui_OutputFcn',  @VisualizeWormDataSplit3D_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before VisualizeWormDataSplit3D is made visible.
function VisualizeWormDataSplit3D_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to VisualizeWormDataSplit3D (see VARARGIN)

% Choose default command line output for VisualizeWormDataSplit3D
...
% UIWAIT makes VisualizeWormDataSplit3D wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = VisualizeWormDataSplit3D_OutputFcn(hObject, eventdata, handles)
...
wormData=load([imFolder filesep matFiles(iImage).name]);
...
display(num2str(max(wormMask(:))))
    ...
    R=getappdata(0,'registration');
        ...
        zSlice=1;
            ...
            imageIdx=wormData.FrameData.imageIdx;
            zPos=wormData.FrameData.zPos;
            ...
            iImage=imageIdx((zSlice));
            imName=['image' num2str(iImage,'%3.5d') '.tif'];
            flag3d=1;

            switch get(handles.channelSelect,'value')
                case 1
                    baseImg= double(imread([rawImFolder{1} filesep imName]...
                        ,'tif')); %red
                case 2
                    baseImg= double(imread([rawImFolder{2} filesep imName]...
                        ,'tif')); %green
                case 3
                    baseImg=wormMask(:,:,zSlice);
                case 4
                    baseImg=0;
                    for iSlice=1:length(zPos)/2
                     iImage=imageIdx((iSlice));
                     imName=['image' num2str(iImage,'%3.5d') '.tif'];
                     baseImg= baseImg+double(imread([rawImFolder{1} filesep imName]...
                        ,'tif')); %green
                    end
                ...
                case 5 
                    ...
                    for iSlice=1:length(zPos)/2
                     iImage=imageIdx((iSlice));
                     imName=['image' num2str(iImage,'%3.5d') '.tif'];
                     baseImg= max(baseImg,double(imread([rawImFolder{1} filesep imName]...
                        ,'tif'))); %green
                    end
                    baseImg=baseImg/length(zPos);
                otherwise
