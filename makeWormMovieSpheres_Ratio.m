...
imageFolder=[dataFolder filesep 'imageMovieAllBalls6'];
...
behaviorState=ethoTrack(stackIdx);
...
%%
  colorT=G2ColorsLookup(:,stackIdx);
...
colorBalls=Fmap(G2ColorsLookup(:,stackIdx),:);
ballSize=G2(:,stackIdx);
...
scatterMax=max(fiducials/4000);
scatterMin=min(fiducials/4000);
colormap(Fmap)
%h=lcolorbar(0:.5:2);
caxis([0 1.3])
...
refY=linspace(scatterMax(2)+.005,scatterMin(2)-.005, nBalls);
refX=ones(1,nBalls)*(scatterMax(1)+.01);
refZ=zeros(1,nBalls);
...
xlimits=[scatterMin(1)+.03, scatterMax(1)+.04];
ylimits=[scatterMin(2)-.01, scatterMax(2)+.01];



sph=gca;
 %htext=text(ylimits(2),xlimits(1),0,stateText,'parent',sph,'Color',color,'FontSize',20);
 htext2=text(refX+.005,refY,refZ,strread(num2str(refI,'%6.2g'),'%s'),'parent',sph,'Color',[0 0 0],'FontSize',12);
 htext3=text(refX(midBall)+.02,refY(midBall),refZ(midBall),'\DeltaR/R0'...
...
subplot(2,2,3)
   ...
   text(136,136,stateText,'Color',color,'FontSize',12)
   hold off
%eigenPosture
%     subplot(2,2,1);
% %delete(gca)
...
frameCounter=frameCounter+1;
    ...
    display(['completed stack:' num2str(stackIdx) ' in ' num2str(toc) 's']);
        catch ME
