function varargout = scan3Dtiffseries(varargin)
% SCAN3DTIFFSERIES MATLAB code for scan3Dtiffseries.fig
%      SCAN3DTIFFSERIES, by itself, creates a new SCAN3DTIFFSERIES or raises the existing
%      singleton*.
%
%      H = SCAN3DTIFFSERIES returns the handle to a new SCAN3DTIFFSERIES or the handle to
%      the existing singleton*.
%
%      SCAN3DTIFFSERIES('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SCAN3DTIFFSERIES.M with the given input arguments.
%
%      SCAN3DTIFFSERIES('Property','Value',...) creates a new SCAN3DTIFFSERIES or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before scan3Dtiffseries_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to scan3Dtiffseries_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help scan3Dtiffseries

% Last Modified by GUIDE v2.5 13-May-2015 12:29:14

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @scan3Dtiffseries_OpeningFcn, ...
    'gui_OutputFcn',  @scan3Dtiffseries_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before scan3Dtiffseries is made visible.
function scan3Dtiffseries_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to scan3Dtiffseries (see VARARGIN)

% Choose default command line output for scan3Dtiffseries
...
% UIWAIT makes scan3Dtiffseries wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = scan3Dtiffseries_OutputFcn(hObject, eventdata, handles)
...
set(handles.currentImage,'String',imFiles(iImage).name);
...
pointsi=TrackData{iImage};
pointsi=pointsi(abs(pointsi(:,3)-iSlice)<=10 & ~isnan(pointsi(:,4)),[2 1 3:end]);
pointID=pointsi(:,4);
pointID=cellstr(num2str(pointID));
pointsRegion=pointsi(:,5);
pointsIdx=0:5;
for i=1:length(pointsIdx);
    regionSelect=pointsRegion==pointsIdx(i);
scatter(handles.axes1,pointsi(regionSelect,1),pointsi(regionSelect,2),shapeVector(i));
text(pointsi(regionSelect,1),pointsi(regionSelect,2),pointID(regionSelect),...
    'color','w','parent',handles.axes1);
        ...
        %show all points
pointsi=TrackData{iImage};
pointsi=pointsi(abs(pointsi(:,3)-iSlice)<=10,[2 1 3:end]);
pointID=pointsi(:,4);
pointID=cellstr(num2str(pointID));
...
text(pointsi(regionSelect,1),pointsi(regionSelect,2),pointID(regionSelect),...
    ...
    nselect=str2double(get(handles.trackNeuron,'String'));
       regionSelect=pointsi(:,4)==nselect;
