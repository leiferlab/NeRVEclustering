function [energy, grad] = gmmreg_L2_tps_costfunc_jn(param, init_affine, basis,  kernel, scene, scale, alpha, beta, n, d, fiducial_indices, spring_constant)
    %%=====================================================================
...
% 
% [~, g1] = gaussOverlap(model_excluding_fiducials,model_excluding_fiducials,scale);
% [~, g2] = gaussOverlap(model_excluding_fiducials,scene_excluding_fiducials,scale);

[f1, g1] = GaussTransform(model,model,scale);
[f2, g2] = GaussTransform(model,scene,scale);

[~, g1] = gaussOverlap(model,model,scale);
[~, g2] = gaussOverlap(model,scene,scale);
...
for i = 1:size(g, 1)
    if ismember(i, fiducial_indices(:,1))
        ...
        g(i,:) = spring_constant * difference+non_fiducial_g(i, :);
